      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-[#1a1a1a] rounded-xl p-6 md:p-8 flex flex-col justify-center">
            <span className="text-gray-500 text-sm mb-3">{FEATURED_BLOG.date}</span>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {FEATURED_BLOG.tags.map((tag) => (
                <span 
                  key={tag}
                  className="px-3 py-1 text-xs font-medium text-gray-300 border border-gray-600 rounded"
                >
                  {tag}
                </span>
              ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {SMALL_BLOGS.map((blog, index) => (
            <div key={index} className="group cursor-pointer">
              ...
              {/* Content */}
              <span className="text-gray-500 text-xs mb-2 block">{blog.date}</span>
              ...
              <div className="flex flex-wrap gap-2">
                {blog.tags.map((tag) => (
                  <span 
                    key={tag}
                    className="px-2 py-0.5 text-[10px] font-medium text-gray-400 border border-gray-700 rounded"
                  >
                    {tag}
                  </span>
                ))}
