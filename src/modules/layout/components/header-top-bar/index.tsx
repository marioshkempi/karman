...
const HeaderTopBar = ({ topbarText, isScrolled = false }: HeaderTopBarProps) => {
    ...
    <div className={`w-full transition-colors duration-300 ${
      ...
      <div className="max-w-[1350px] mx-auto px-6 py-1 flex items-center justify-between">
        ...
        <div className={`flex items-center gap-4 text-[13px] ${
          ...
          {/* Phone */}
          <a href="tel:2410232019" className={`flex items-center gap-2 transition-colors ${
            isScrolled ? "hover:text-gray-900" : "hover:text-white/80"
          }`}>
          ...
          <div className="flex items-center gap-3 ml-2">
            {/* Facebook */}
            <a href="#" className={`transition-colors ${isScrolled ? "hover:text-gray-900" : "hover:text-white/80"}`} aria-label="Facebook">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/>
              </svg>
            </a>
            {/* Instagram */}
            <a href="#" className={`transition-colors ${isScrolled ? "hover:text-gray-900" : "hover:text-white/80"}`} aria-label="Instagram">
            ...
            {/* TikTok */}
            <a href="#" className={`transition-colors ${isScrolled ? "hover:text-gray-900" : "hover:text-white/80"}`} aria-label="TikTok">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z"/>
              </svg>
            </a>
            {/* LinkedIn */}
            <a href="#" className={`transition-colors ${isScrolled ? "hover:text-gray-900" : "hover:text-white/80"}`} aria-label="LinkedIn">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div className="flex items-center">
          ...
          <div className="relative">
            <button 
              ...
              className={`flex items-center gap-1.5 text-[13px] transition-colors border rounded px-3 py-1 ${
                isScrolled 
                  ? "text-gray-700 border-gray-300 hover:text-gray-900" 
                  : "text-white border-white/30 hover:text-white/80"
              }`}
            >
              <span>ΕΛΛΗΝΙΚΑ</span>
              <ChevronDown size={14} />
